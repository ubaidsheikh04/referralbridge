...
import { addDoc, collection, getDocs, query, where } from "firebase/firestore"; // Added getDocs, query, where
import { firebaseApp, db } from "@/services/firebase";
import { useRouter } from 'next/navigation';
import Script from 'next/script';
import ReferralRequestTile from '@/components/ReferralRequestTile'; // For displaying requests
...
const RequestReferralPage = () => {
  ...
  // States for "View My Requests" feature
  ...
  const createRazorpayOrder = async (referralData: ReferralFormValues) => {
    ...
    try {
      ...
      if (!data.orderId || !data.keyId) { throw new Error("Received invalid order data from server."); }
  ...
  const handleRazorpayPayment = (keyId: string, orderId: string, referralDataOnPayment: ReferralFormValues) => {
    ...
    const options = {
      ...
      handler: async function (response: any) {
        ...
        try {
          ...
          if (resumeFiles instanceof FileList && resumeFiles.length === 1) {
            try { uploadedResumeUrl = await uploadFile(resumeFiles[0]); } catch (uploadError: any) {
          ...
          } else { console.warn("Resume file was not in the expected format after payment verification."); }
      ...
      prefill: { name: referralDataOnPayment.name, email: referralDataOnPayment.email },
      notes: { name: referralDataOnPayment.name, email: referralDataOnPayment.email, targetCompany: referralDataOnPayment.targetCompany, jobId: referralDataOnPayment.jobId },
      theme: { color: "#FDB813" },
      modal: {
        ondismiss: function () {
  ...
  const loadRazorpayScript = (callback?: () => void) => {
    ...
    if (existingScript && (window as any).Razorpay) { callback?.(); return; }
    if (existingScript) {
      existingScript.addEventListener('load', () => {
        if ((window as any).Razorpay) callback?.();
        else { toast({ variant: 'destructive', title: 'Error', description: 'Payment gateway failed to initialize. Please refresh.' }); setIsLoading(false); setPaymentInProgress(false); }
      });
      return;
    }
  ...
  // "View My Requests" specific functions
  ...
  return (
    <>
      <Script id="razorpay-checkout-js" src="https://checkout.razorpay.com/v1/checkout.js" strategy="lazyOnload" onLoad={() => console.log("Razorpay script loaded.")} />
